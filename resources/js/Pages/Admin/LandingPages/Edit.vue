...
<template>
  ...
  <AdminLayout>
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- <pre>{{ form }}</pre> -->
      <form @submit.prevent="submitForm" class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <!-- Left Side: Settings -->
        <div class="lg:col-span-1 space-y-6">
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 space-y-6">
              ...
              <!-- New Fields -->
              <div v-if="form.is_enabled_faq" class="space-y-3">
                <div>
                  <label for="faq_style" class="block text-sm font-medium text-gray-700 mb-2">FAQ Style</label>
                  <select
                    v-model="form.faq_style"
                    id="faq_style"
                ...
                <div>
                  <label for="faq_order" class="block text-sm font-medium text-gray-700 mb-2">FAQ Order</label>
                  <input
                    v-model="form.faq_order"
                    type="number"
                    id="faq_order"
                    min="1"
                    max="10"
              ...
              <div v-if="form.is_enabled_review" class="space-y-3">
                <div>
                  <label for="review_style" class="block text-sm font-medium text-gray-700 mb-2">Review Style</label>
                  <select
                    v-model="form.review_style"
                    id="review_style"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                  >
                    <option value="">Select Style</option>
                ...
                <div>
                  <label for="review_order" class="block text-sm font-medium text-gray-700 mb-2">Review Order</label>
                  <input
                    v-model="form.review_order"
                    type="number"
                    id="review_order"
                    min="1"
                    max="10"
              ...
              <div v-if="form.is_enabled_cta" class="space-y-3">
                <div>
                  <label for="cta_style" class="block text-sm font-medium text-gray-700 mb-2">CTA Style</label>
                  <select
                ...
                <div>
                  <label for="cta_order" class="block text-sm font-medium text-gray-700 mb-2">CTA Order</label>
                  <input
                    v-model="form.cta_order"
                    type="number"
                    id="cta_order"
                    min="1"
                    max="10"
              ...
              <div class="space-y-3">
                <div>
                  <label for="youtube_video_order" class="block text-sm font-medium text-gray-700 mb-2">YouTube Video Order</label>
                  <input
                    v-model="form.youtube_video_order"
                    type="number"
                    id="youtube_video_order"
                    min="1"
                    max="10"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                    placeholder="1-10"
                ...
                <div>
                  <label for="upload_video_order" class="block text-sm font-medium text-gray-700 mb-2">Upload Video Order</label>
                  <input
                    v-model="form.upload_video_order"
                    type="number"
                    id="upload_video_order"
                    min="1"
                    max="10"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                    placeholder="1-10"
        ...
        <div class="lg:col-span-3 space-y-8">
          ...
          <!-- CTA Section -->
          <div v-if="form.is_enabled_cta" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
          ...
          <!-- FAQs -->
          <div v-if="form.is_enabled_faq" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
          ...
          <!-- Review Images -->
          <div v-if="form.is_enabled_review" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
          ...
          <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
            ...
            >
              {{ processing ? 'Saving...' : 'Update Landing Page' }}
            </button>
          </div>
        </div>
      </form>
...
const loadPage = (page) => {
  router.get(route('admin.landing-pages.edit', props.landingPage.id), { page }, {
    preserveState: true,
    preserveScroll: true,
  });
};
...
const populateForm = () => {
  ...
  // Initialize as empty arrays for new files
  form.value.product_gallery = [];
  form.value.review_images = [];

  if (lp.hero_image) {
    previewImages.value.hero_image = `${lp.hero_image}`;
  }
  if (lp.feature_image) {
    previewImages.value.feature_image = `${lp.feature_image}`;
  }
  if (lp.product_gallery) {
    const gallery = Array.isArray(lp.product_gallery) ? lp.product_gallery : JSON.parse(lp.product_gallery);
    previewImages.value.product_gallery = gallery.map(img => `${img}`);
  }
  if (lp.review_images) {
    const reviews = Array.isArray(lp.review_images) ? lp.review_images : JSON.parse(lp.review_images);
    previewImages.value.review_images = reviews.map(img => `${img}`);
  }

  if (lp.settings) {
    form.value.primary_color = lp.settings.primary_color || '#4f46e5';
    form.value.secondary_color = lp.settings.secondary_color || '#10b981';
    form.value.is_enabled_faq = lp.settings.is_enabled_faq ? 1 : 0;
    form.value.is_enabled_review = lp.settings.is_enabled_review ? 1 : 0;
    form.value.is_enabled_cta = lp.settings.is_enabled_cta ? 1 : 0;
    form.value.faq_style = lp.settings.faq_style || '';
    form.value.review_style = lp.settings.review_style || '';
    form.value.cta_style = lp.settings.cta_style || '';
    form.value.faq_order = lp.settings.faq_order || '';
    form.value.review_order = lp.settings.review_order || '';
    form.value.cta_order = lp.settings.cta_order || '';
    form.value.youtube_video_order = lp.settings.youtube_video_order || '';
    form.value.upload_video_order = lp.settings.upload_video_order || '';
  }

  if (lp.products) {
    form.value.products = lp.products.map((product) => product.product_id);
  }
};
...
const removeGalleryImage = (index) => {
  const imageUrl = previewImages.value.product_gallery[index];

  // If it's an existing image (URL string, not blob), track it for deletion
  if (isEdit.value && imageUrl && !imageUrl.startsWith('blob:')) {
    // Use the full path as stored in database
    form.value.deleted_gallery_images.push(imageUrl);
  }

  // Remove from both form and preview arrays
  if (form.value.product_gallery[index] instanceof File) {
...
const removeReviewImage = (index) => {
  const imageUrl = previewImages.value.review_images[index];

  // If it's an existing image (URL string, not blob), track it for deletion
  if (isEdit.value && imageUrl && !imageUrl.startsWith('blob:')) {
    // Use the full path as stored in database
    form.value.deleted_review_images.push(imageUrl);
  }

  // Remove from both form and preview arrays
  if (form.value.review_images[index] instanceof File) {
...
const cancel = () => {
  router.visit(route('admin.landing-pages.index'));
};

const submitForm = () => {
  processing.value = true;

  console.log('Current form.value before sending:', form.value);

  // Prepare data object for Inertia
  const data = {
    _method: 'PATCH', // Important: Add method spoofing
    title: form.value.title,
    button_text: form.value.button_text,
    youtube_url: form.value.youtube_url,
    feature_button_text: form.value.feature_button_text,
    cta_title: form.value.cta_title,
    cta_short_description: form.value.cta_short_description,
    cta_button_text: form.value.cta_button_text,
    primary_color: form.value.primary_color,
    secondary_color: form.value.secondary_color,
    is_enabled_faq: form.value.is_enabled_faq,
    is_enabled_review: form.value.is_enabled_review,
    is_enabled_cta: form.value.is_enabled_cta,
    faq_style: form.value.faq_style,
    review_style: form.value.review_style,
    cta_style: form.value.cta_style,
    faq_order: form.value.faq_order,
    review_order: form.value.review_order,
    cta_order: form.value.cta_order,
    youtube_video_order: form.value.youtube_video_order,
    upload_video_order: form.value.upload_video_order,
    product_features_list: form.value.product_features_list,
    faqs: form.value.faqs,
    products: form.value.products,
    deleted_gallery_images: form.value.deleted_gallery_images,
    deleted_review_images: form.value.deleted_review_images,
  };

  // Add files separately
  if (form.value.hero_image instanceof File) {
    data.hero_image = form.value.hero_image;
  }
  if (form.value.feature_image instanceof File) {
    data.feature_image = form.value.feature_image;
  }
  if (form.value.upload_video instanceof File) {
    data.upload_video = form.value.upload_video;
  }

  // Add multiple files (only new File objects)
  if (form.value.product_gallery && form.value.product_gallery.length > 0) {
    const newGalleryFiles = form.value.product_gallery.filter(file => file instanceof File);
    if (newGalleryFiles.length > 0) {
      data.product_gallery = newGalleryFiles;
    }
  }
  if (form.value.review_images && form.value.review_images.length > 0) {
    const newReviewFiles = form.value.review_images.filter(file => file instanceof File);
    if (newReviewFiles.length > 0) {
      data.review_images = newReviewFiles;
    }
  }
  ...
  router.post(route('admin.landing-pages.update', props.landingPage.id), data, {
    forceFormData: true,
    preserveScroll: true,
    onSuccess: () => {
