...
const loadPage = (page) => {
  router.get(route('admin.landing-pages.create'), { page }, {
    preserveState: true,
    preserveScroll: true,
  });
};
...
const populateForm = () => {
  ...
  if (lp.settings) {
    ...
    form.value.is_enabled_faq = lp.settings.is_enabled_faq;
    form.value.is_enabled_review = lp.settings.is_enabled_review;
    form.value.is_enabled_cta = lp.settings.is_enabled_cta;
  }
...
const submitForm = () => {
  ...
  router.post(route(routeName, isEdit.value ? props.landingPage.id : null), data, {
    forceFormData: true,
    onSuccess: () => {
